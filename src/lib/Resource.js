...
function Resource(_config) {
    this._config = _config;
    // 如果有actions，生成这些额外的方法，
    //这些方法是实例的方法，并不是公有方法（原型链上的方法,也就是说每个实例的这些方法可能都不一样）
    if (_config.actions && _config.actions.length > 0) {
        for(var i=0;i<_config.actions.length;i++){
            // 此处必须用let声明，如果用var在for循环中声明变量会有问题
            let v = _config.actions[i];
            // 如果有形如/:id的参数
            if(v.url.indexOf('/:')!=-1){
                ...
                this[v.name] = function (option) {
                    ...
                    var selfOption = {
                        url: this.baseUrl + _config.url + '/' + actionName + '/' + option.params[actionParam],
                        method: v.method
                    };
                ...
                this[v.name] = function (option) {
                    var selfOption = {
                        url: this.baseUrl + _config.url + '/' + v.url,
                        method: v.method
                    };
                    option = Object.assign({}, this.baseOption, selfOption, option);
                    return this.handle(option);
...
Resource.prototype.query = function (option) {
    var selfOption = {
        url: this.baseUrl + this._config.url,
        method: 'GET'
    };
    option = Object.assign({}, this.baseOption, selfOption, option);
    return this.handle(option);
...
Resource.prototype.get = function (option) {
    var selfOption = {
        url: this.baseUrl + this._config.url + '/' + option.params.id,
        method: 'GET'
    };
    option = Object.assign({}, this.baseOption, selfOption, option);
    return this.handle(option);
...
Resource.prototype.update = function (option) {
    ...
    var selfOption = {
        url: this.baseUrl + this._config.url + '/' +  id,
        method: 'PUT'
    };
    option = Object.assign({}, this.baseOption, selfOption, option);
    return this.handle(option);
